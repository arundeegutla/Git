...
public class catandmice {
    ...
    public static Mice[] MICE;
    public static double NEGVEL = -1;
    private static void solve() {
        MICE = new Mice[in.nextInt()];
        for (int i = 0; i < MICE.length; i++)
            MICE[i] = new Mice(in.nextInt(), in.nextInt(), in.nextInt());
        NEGVEL = in.nextDouble();
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    public static boolean tsp(double initial) {
        int n = MICE.length;
        double[][] memo = new double[1<<n][n];
        for (double[] ds : memo) Arrays.fill(ds, -1);
        for (int i = 0; i < n; i++) 
            if (tspHelper(1<<i, i, initial, memo) <= MICE[i].hides)
                return true;
        return false;
    }
    
    public static double tspHelper(int visited, int current, double initial, double[][] memo) {

        if (memo[visited][current] != -1)
            return memo[visited][current];

        if (visited == ((1<<MICE.length) - 1)) {
            double dist = MICE[current].loc.distance(0, 0);
            double time = dist/initial;
            return time <= MICE[current].hides ? time : 1e9;
        }
    
        int numtovisited = MICE.length - Integer.bitCount(visited);
        double neg = Math.pow(NEGVEL, numtovisited);
        double curVel = initial * neg;

        double minTime = 1e9;
        for (int i = 0; i < MICE.length; i++) {
            if (!visited(i, visited)) {
                double dist = MICE[current].loc.distance(MICE[i].loc);
                double time = dist/curVel;
                double curtime = tspHelper(visited | (1<<i), i, initial, memo) + time;
                minTime = Math.min(minTime, curtime<=MICE[current].hides ? curtime : 1e9);
            }
        }

        return memo[visited][current] = minTime;
    }

    private static boolean visited(int i, int visited) {
...
class MyUtils {
    ...
    public static long tsp(long[][] dist) {
        int n = dist.length;
        ...
        for (int i = 0; i < n; i++){
            long thisShortest = tspHelper(1<< i, i, dist, memo) + dist[0][i];
            shortest = Math.min(shortest, thisShortest);
        }
        return shortest;
    }
    
    public static long tspHelper(int visited, int current, long[][] dist, long[][] memo) {
        if (memo[visited][current] != -1)
            return memo[visited][current];
        ...
        for (int i = 0; i < dist.length; i++)
            if ((visited & (1<<i)) == 0) // only go to unvisited
                min = Math.min(min, tspHelper(visited | (1<<i), i, dist, memo) + dist[current][i]);
...
class Mice{
    Point loc;
    int hides;
    Mice(int x, int y, int hides) {this.loc=new Point(x, y);this.hides=hides;};
    @Override
