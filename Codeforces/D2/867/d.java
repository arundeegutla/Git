...
public class d {
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    private static void go(int at, int n, int[] perm, boolean[] used) {
        if (at==n) {
            check(perm);
            return;
        }

        for (int i = at==0?n-1:1; i < n; i++) {
            if (!used[i]) {
                used[i] = true;
                perm[at] = i;
                go(at+1, n, perm, used);
    ...
    private static void check(int[] perm) {
        int n = perm.length - 1;
        boolean[] newArr = new boolean[perm.length];

        int[] psum = new int[perm.length];
        for (int i = 1; i < psum.length; i++) psum[i] = perm[i] + psum[i-1];
