...
public class c {
    ...
    private static void solve() {
        ...
        while (numCases-->0) {
            ...
            for (int i = 0; i < books.length; i++) {
                books[i] = new Book(in.nextInt(), in.next());
            }
            long[][][] memo = new long[numBooks+1][2][2];
            for (long[][] ls : memo) {
                for (long[] ls2 : ls) {
                    Arrays.fill(ls2, -1);
                }
            }
            long time = go(0, 0, 0, books, memo);
            if (time==(long)1e17) {
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    private static long go(int at, int one, int two, Book[] books, long[][][] memo) {
        if(one==1&&two==1)
            return 0;
        if(at == books.length)
            return (long)1e17;
        if (memo[at][one][two] != -1) {
            // System.out.println("memp!");
            return memo[at][one][two];
        }
        long dont = go(at+1, one, two, books, memo);
        long take = go(at+1, one==0?books[at].one:one, two==0?books[at].two:two, books, memo) + books[at].minutes;

        return memo[at][one][two] = Math.min(dont, take);
    }
