...
public class g {
    ...
    private static void solve() {
        ...
        long[][] memo = new long[2024][2024];

        HashMap<Long, Point> map = new HashMap<>();
        
        // nice rectangle matrix.
        int start = 1;
        for (int i = 1; i <= 2023; i++) {
            memo[i][1] = start;
            map.put((long)start, new Point(i, 1));
            for (int j = 2; j <= 2023 && memo[i][j-1] + (j+(i-1)) <= (int)1e6; j++) {
                memo[i][j] = memo[i][j-1] + (j+(i-1));
                map.put(memo[i][j-1] + (j+(i-1)), new Point(i, j));
            }
            start += i;
        }

        // prefix horizontal and square them.
        ...
        int n = in.nextInt();
        while (n-->0) {
            Point p = map.get(in.nextLong());
            out.println(memo[p.x][p.y]);
        }
