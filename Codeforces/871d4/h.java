...
public class h {
    ...
    private static void solve() {
        ...
        while (numCases-->0) {
            ...
            long[][] memo = new long[n+1][64];
            for (long[] ls : memo)
                Arrays.fill(ls, -1);

            System.out.println(go(0, 63, k, arr, memo));
        }
    }
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    private static long go(int at, int and, int k, int[] arr, long[][] memo) {
        if (at == arr.length)
            return 0;
        if (memo[at][and] != -1) {
            return memo[at][and];
        ...
        long count = 0;
        count += go(at+1, and&arr[at], k, arr, memo) + (Integer.bitCount(and&arr[at])==k?1:0);
        count %= MOD;
        count += go(at+1, and, k, arr, memo);
        count %= MOD;

        return memo[at][and] = count;
    }
