    ...
    private static void solve() {
        ...
        while (numCases-->0) {
            ...
            for (int i : map.keySet()) {
                // b
                for (int b = 1; (long)i*b*b <= max; b++) {

                    // System.out.println(("i: ") + i + " b: " + b + " j: " + (i*b) + " k: " + (i*b*b));
                    int j = i*b;
                    Integer second = map.get(j);

                    if (second == null) {
                        Integer h = map.higherKey(i*b);
                        if (h == null) break;
                        if(h%i > 0) h += (i-(h%i));

                        // System.out.println(i*b + " didnt work; jumping to : " + h + " " + (h/i) + " max: " + ((long)i*b*b));
                        b = (h/i)-1;
                        // System.out.println((i*b*b) + "<=" + max);
                    } else {
                        int k = j*b;
 
                        Integer first = map.get(i);
                        Integer third = map.get(k);
