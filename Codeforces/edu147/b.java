...
public class b {
    ...
    private static void solve() {
        ...
        while (numCases-->0) {
            // Insert code here.
            int n = in.nextInt();
            int[] old = in.nextIntArray(n);
            int[] newA = in.nextIntArray(n);


            int leftNotMatch = 0;
            int rightNotMatch = n-1;
            while (old[leftNotMatch] == newA[leftNotMatch])
                leftNotMatch++;
            while (old[rightNotMatch] == newA[rightNotMatch])
                rightNotMatch--;


            while (leftNotMatch > 0 && newA[leftNotMatch-1] <= newA[leftNotMatch]) {
                leftNotMatch--;
            }
            while (rightNotMatch < n-1 && newA[rightNotMatch+1] >= newA[rightNotMatch]) {
                rightNotMatch++;
            }

            System.out.println((leftNotMatch+1) + " " + (rightNotMatch+1));
