    ...
    private static void solve() {
        ...
        while (numCases-->0) {
            ...
            for (int i = 0; i < s.length(); i++) {
                if (s.charAt(i)=='L') {
                    tot += i;
                    pq.add(new Person(i, (n-(i+1))));
                } else {
                    tot += (n-(i+1));
                    pq.add(new Person((n-(i+1)), i));
                }
            }

            for (int i = 1; i <= n; i++) {
                tot = Math.max(tot, tot + pq.poll().delta);
                out.print(tot+" ");
            }
...
class Person implements Comparable<Person>{
    ...
    int potential;
    int delta;
    Person(int curLooking, int potential) {
        this.curLooking = curLooking;
        this.potential = potential;
        delta = potential - curLooking;
    }
    @Override
