...
public class c {
    ...
    private static void solve() {
        ...
        loop:while (numCases-->0) {
            ...
            // Make a line graph. any point in between highest and lowest can be deleted.
            for (int i = 1; i+1 < arr.length; i++) {
                if((arr[prev] <= arr[i] && arr[i] <= arr[i+1]) || (arr[prev] >= arr[i] && arr[i] >= arr[i+1])) {
                    numDeletes++;
                } else {
                    prev = i;
                }
            ...
            out.println(n-numDeletes);
        }
    }
    
    private static boolean allSame(long[] arr) {
        long num = arr[0];
        for (long l : arr) {
            if(l!=num)
                return false;
