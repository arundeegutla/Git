...
public class a {
    ...
    private static void solve() {
        ...
        for (int i = 0; i < q; i++) {
            ...
            out.println(getBigSquare(grid, from, to));
        }
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    private static long getBigSquare(int[][] grid, int from, int to) {
        int n = grid.length;
        int max = 0;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (grid[i][j] < from) continue;
                if (grid[i][j] > to) break;
                int k = Math.max(max-1, 0);
                while (k < Math.min(n-i, n-j) && grid[i+k][j+k] <= to) k++;
                k--;
                if(max < k+1) max = k+1;
                else break;
