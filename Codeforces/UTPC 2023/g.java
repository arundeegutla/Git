...
public class g {
    ...
    private static void solve() {
        ...
        Graph g = new Graph(n);
        for (int i = 1; i < n; i++) {
            g.addEdge(in.nextInt()-1, in.nextInt()-1, 1, true, i-1);
        }

        boolean[] ids = new boolean[n-1];
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    private static void visit(Point point, Graph g, boolean[] ids, HashSet<Point> set) {
        // System.out.println("checking " + from + " --> "+ to);
        HashSet<Vertex> visited = new HashSet<>();
        if (!set.contains(point)) return;
        DFS(g.nodes.get(point.x), g.nodes.get(point.y), ids, visited, set);
    }

    private static boolean DFS(Vertex f, Vertex t, boolean[] ids, HashSet<Vertex> visited, HashSet<Point> set) {
        // System.out.println(f.id + " --> " + t.id);
        if (f.equals(t)) return true;
        visited.add(f);
        for (Edge e : f.edges) {
            boolean b = ids[e.id];
            ids[e.id] = true;
            if (!visited.contains(e.to) && DFS(e.to, t, ids, visited, set)) {
                set.remove(new Point(f.id, e.to.id));
                set.remove(new Point(e.to.id, t.id));
                return true;
            }
            ids[e.id] = b;
        }
...
class Graph {
    ...
    public void addEdge(int from, int to, long weight, boolean biDirected, int id) {
        nodes.get(from).addEdge(nodes.get(to), weight, id);
        if (biDirected)
            nodes.get(to).addEdge(nodes.get(from), weight, id);
    }
}
...
class Vertex {
    ...
    public void addEdge(Vertex to, long weight, int id) {
        edges.add(new Edge(this, to, weight, id));
    }
    @Override
    public int hashCode() {
...
class Edge {
    ...
    long weight;
    int id;
    Edge (Vertex from, Vertex to, long weight, int id) {
        this.from = from;
        this.to = to;
        this.weight = weight;
        this.id = id;
