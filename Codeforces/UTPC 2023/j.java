...
public class j {
    ...
    private static void solve2() {
        if ((in.nextInt() ^ in.nextInt() ^ in.nextInt()) != 0)
    ...
    private static void solve() {
        ...
        Boolean[][][][] memo = new Boolean[2][a+1][b+1][c+1];
        if (go(0, a, b, c, memo))
            System.out.println("Yes");
        else
            System.out.println("No");
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    private static boolean go(int turn, int a, int b, int c, Boolean[][][][] memo) {
        if (a==0&&b==0&&c==0)
            return turn != 0;

        if (memo[turn][a][b][c] != null)
            return memo[turn][a][b][c];
        ...
        if (turn == 0) {
            boolean x = false;
            for (int i = 1; i <= max; i++) {
                if(i <= a) x |= go(1, a-i, b, c, memo);
                if(i <= b) x |= go(1, a, b-i, c, memo);
                if(i <= c) x |= go(1, a, b, c-i, memo);
                if(i <= min) x |= go(1, a-i, b-i, c-i, memo);
            }
            return memo[turn][a][b][c] = x;
        } 

        boolean x = true;
        for (int i = 1; i <= max; i++) {
            if(i <= a) x &= go(0, a-i, b, c, memo);
            if(i <= b) x &= go(0, a, b-i, c, memo);
            if(i <= c) x &= go(0, a, b, c-i, memo);
            if(i <= min) x &= go(0, a-i, b-i, c-i, memo);
        }
