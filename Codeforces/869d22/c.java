...
public class c {
    ...
    private static void solve() {
        ...
        int[] consecFreq = new int[n+1];
        for (int i = 0; i+2 < n; i++) {
            if(arr[i]>=arr[i+1]&&arr[i+1]>=arr[i+2])
                consecFreq[i+3]++;
            consecFreq[i+3] += consecFreq[i+2];
        }

        // System.out.println("   " + Arrays.toString(arr));

        // System.out.println(Arrays.toString(consecFreq));
        for (int i = 0; i < q; i++) {
            int left = in.nextInt();
            ...
            // System.out.println(left+" " + right + " --> " + (right+1-left) + " - " + (consecFreq[right]-consecFreq[left+1]));

            int len = (right+1-left) - (consecFreq[right]-consecFreq[left+1]);
            out.println(len);

        }


        // int[] decreasingPrefix = new int[n+1];
        // int[] increasingPrefix = new int[n+1];

        // increasingPrefix[1] = 1;
        // for (int i = 1; i < n; i++) {
        //     if(arr[i] > arr[i-1]) {
        //         increasingPrefix[i+1] = 1;
        //     } else {
        //         decreasingPrefix[i+1] = 1;
        //     }
        //     increasingPrefix[i+1] += increasingPrefix[i];
        //     decreasingPrefix[i+1] += decreasingPrefix[i];
        // }

        // for (int i = 0; i < q; i++) {
        //     int left = in.nextInt();
        //     int right = in.nextInt();
        //     int numIncreasing = increasingPrefix[right]-increasingPrefix[left];
        //     int numDecreasing = decreasingPrefix[right]-decreasingPrefix[left-1];
        //     System.out.println(left+" " + right + " --> " + numIncreasing + " " + numDecreasing);
        //     System.out.println(numIncreasing+1+Math.min(numDecreasing, 2));
        // }

        // System.out.println(Arrays.toString(increasingPrefix));
        // System.out.println(Arrays.toString(decreasingPrefix));
