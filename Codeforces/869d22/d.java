...
public class d {
    ...
    private static void solve() {
        ...
        loop:while (numCases-->0) {
            ...
            for (int i = 0; i < m; i++) {
                g.addEdge(in.nextInt()-1, in.nextInt()-1, 1, true);
            }

            // this is u
            HashSet<Edge> list = new HashSet<>();
            for (int u = 0; u < n; u++) {
                if(g.nodes.get(u).edges.size() >= 4 && go(u, g, list)){
                    out.println("YES");
                    out.println(list.size());
                    for (Edge edge : list) out.println(edge);
                    continue loop;
                } 
            }
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    private static boolean go(int u, Graph g, HashSet<Edge> list) {

        HashMap<Vertex, Integer> ids = new HashMap<>();
        Queue<Vertex> q = new ArrayDeque<>();

        int[] prev = new int[g.nodes.size()];
        int i = 0;
        for (Edge e : g.nodes.get(u).edges) {
            q.add(e.to);
            ids.put(e.to, i++);
            prev[e.to.id] = u;
        }
        ...
        while (!q.isEmpty()) {
            ...
            for (Edge e : v.edges) {
                ...
                if(e.to.id == u) continue;

                if (!ids.containsKey(e.to)) {
                    ids.put(e.to, id);
                    prev[e.to.id] = v.id;
                    q.add(e.to);
                } else if (ids.get(e.to) != id) {

                    int mine = id, other = ids.get(e.to);
                    list.add(new Edge(v, e.to, 1));
                    addEdges(u, v.id, list, prev, g);
                    addEdges(u, e.to.id, list, prev, g);
                    int count = 0;
                    for (Edge ex : g.nodes.get(u).edges) {
                        if(ids.get(ex.to) != mine && ids.get(ex.to) != other) {
                            list.add(ex);
                            count++;
                            if(count==2) break;
                        }
    ...
    private static void addEdges(int u, int id, HashSet<Edge> list, int[] prev, Graph g) {
        int here = id;

        while (here != u) {
            list.add(new Edge(g.nodes.get(prev[here]), g.nodes.get(here), 1));
            here = prev[here];
        }
    }
