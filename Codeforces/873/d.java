            ...
            for (int i = 0; i < arr.length; i++) {
                ...
                for (int j = i+1; j < arr.length; j++) {
                    if(arr[j] >= max){
                        sum += index-minDist;
                        max = arr[j];
                        System.out.println("adding: " +  (index-minDist));
                        continue;
                    }

                    index = j;
                    System.out.println("    Comparing: " + arr[j]);
                    int cur = j-1;
                    while (cur >= i && arr[cur]>arr[j])
                        cur--;
                    cur++;
                    if(cur <= minDist) {
                        minDist = cur;
                    }
                    System.out.println("        switching to " + (minDist+1));
                    
                    if(j-minDist>0)
                        System.out.println("        " + (j+1) + " " + (minDist+1) + " ---> " + (j-minDist));
                    sum += j-minDist;
                }
            }


            out.println(sum + " " + Arrays.toString(arr));
