...
public class c {
    ...
    private static void solve() {
        ...
        while (numCases-->0) {
            ...
            for (int i = 0; i < n; i++) 
                NumberTheory.getPrimeFactors(in.nextInt(), primes);

            int len = 0;
            int notUsed = 0;
            for (var pair : primes.entrySet()) {
                len += (pair.getValue()/2);
                notUsed += (pair.getValue()%2);
            }
            System.out.println(len+(notUsed/3));
        }
    }
...
class NumberTheory {
     ...
     */
    public static void getPrimeFactors(long N, HashMap<Long, Integer> map) {
