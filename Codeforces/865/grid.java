...
public class grid {
    ...
    private static void solve() {
        ...
        while (numCases-->0) {
            ...
            int[][] grid = new int[2][numColumns];


            int count = 2*numColumns + 2;

            for (int i = 0; i < (2*numColumns)/2; i+=2)
                grid[i/numColumns][i%numColumns] = count -= 2;

            count = numColumns -1;
            for (int i = numColumns+1; i < (2*numColumns); i+=2)
                grid[i/numColumns][i%numColumns] = count+=2;


            
            count = 0;
            for (int i = 1; i < (2*numColumns)/2; i+=2)
                grid[i/numColumns][i%numColumns] = count += 2;

            count = -1;
            for (int i = numColumns; i < (2*numColumns); i+=2)
                grid[i/numColumns][i%numColumns] = count+=2;
